...
func run(m *testing.M) int {
	...
	db, err := sql.Open("postgres", "host=localhost port=5432 user=rob sslmode=disable dbname=test_user_store")
	if err != nil {
		panic(fmt.Errorf("sql.Open() err = %s", err))
	}
	defer db.Close()
